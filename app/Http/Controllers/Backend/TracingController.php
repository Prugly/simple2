...
class TracingController extends Controller
{
    ...
    public function ver($tramite_id)
    {
        ...
        if(!is_null($tramite->deleted_at)){
            echo 'El tramite no se encuentra disponible';
            exit;
        }
    ...
    public function ver_etapa($etapa_id, $secuencia = 0)
    {
        ...
        //issue https://git.gob.cl/simple/simple/issues/249
        $estado_etapa = $etapa->pendiente == 0 ? 'Completado' : ($etapa->vencida() ? 'Vencida' : 'Pendiente');
        Log::info("# Estado etapa en seguimiento del tte: " . $estado_etapa);

        $data ['etapa'] = $etapa;
        $data ['paso'] = $paso;
        $data ['secuencia'] = $secuencia;
        $data ['estado_etapa'] = $estado_etapa;
