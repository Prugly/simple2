...
use DB;
use App\Rules\NombreUnicoGrupoUsuarios;
...
class ConfigurationController extends Controller
{
    ...
    {
        $validation = ['nombre' => ['required', 'unique:grupo_usuarios']];
        if ($edit) {
            $validation = [
                'nombre' => [
                    'required', new NombreUnicoGrupoUsuarios($grupo_usuarios),
                ]
            ];
        }
    ...
    {
        $data['categorias'] = Categoria::where('cuenta_id',Auth::user()->cuenta_id)->get();
    ...
    public function editar_lista_categoria(Request $request, $id = null)
    {
        ...
        try {
            ...
            $categoria->cuenta_id = Auth::user()->cuenta_id;
