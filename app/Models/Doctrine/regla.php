            ...
            if ($dato) {
                ...
                if (!is_string($dato_almacenado)) {
                    ...
                    if ($evaluar == true && strlen($accesor) == 0) {
                        ...
                        } elseif($result[0]->tipo == 'grid_datos_externos'){
                            // cabecera de tabla obtenida a partir de la definición
                            $cabeceras = json_decode(json_encode($result[0]->extra->columns), true);
                            $tablagde = '<table border="1" cellpadding="1" cellspacing="1"><thead></thead><tbody>';
                            $tablagde .= '<tr>';
                            foreach($cabeceras as $item)
                            {
                                $tablagde .= '<td><strong>'.ucfirst(strtolower($item['header'])).'</strong></td>';
                            }
                            $tablagde .= '</tr>';
                            //Datos de la grilla
                            try{
                                ...
                                foreach($valor_dato as $key => $array)
                                {
                                    ...
                                    {
                                        $tablagde .= '<td>'.$valor.'</td>';
                                    }
                                    $tablagde .= '</tr>';    
                                }
                                $tablagde .= '</tbody></table>';
                                $valor_dato = $tablagde;
                            }catch(Exception $e) {
                                $valor_dato = '';
                                Log::error("Error" . $e);
